...
- [x] 단일 인스턴스 배포 프로파일 구성 (`file + file`)
- [x] 운영 배포 프로파일 구성 (`redis + mysql`)
- [x] 환경별 설정 템플릿 작성 (`dev`, `staging`, `prod`)
- [x] 롤백 절차/운영 런북 초안 작성

## 14. 완료 기준 (MVP)

- [x] 채널/메시지/쓰레드/파일 기능이 `/v1`에서 동작
- [x] user/token 생성이 `/admin/v1`에서만 허용
- [x] Slack/Telegram/Discord 기본 메시지/파일 API 호환 동작
- [x] 선택형 저장소 백엔드 전환이 설정만으로 가능
