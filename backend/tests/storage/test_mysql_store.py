...
def test_mysql_metadata_store_delete_channel_removes_related_records() -> None:
    store = InMemoryMySQLMetadataStore()
    ...
    asyncio.run(store.next_compat_sequence("slack", "channel-a"))

    deleted_channel = asyncio.run(store.delete_channel("channel-a"))

    assert deleted_channel == created_channel
    assert asyncio.run(store.get_channel("channel-a")) is None
    assert asyncio.run(store.get_message("msg-1")) is None
    assert asyncio.run(store.get_message("msg-2")) is None
    assert asyncio.run(store.get_thread("th-1")) is None
    assert (
        asyncio.run(store.get_compat_mapping("slack", "message", "123.000001", "channel-a"))
        is None
    )
